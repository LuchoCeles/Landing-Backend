...
const updateSchedules = async (req, res) => {
  ...
  try {
    const results = await Promise.all(
      schedules.map(async ({ id, dia, horario }) => {
        const schedule = await Schedule.findByPk(id);

        if (!schedule) {
          return { id, success: false, message: 'Horario no encontrado' };
        }
        ...
        await schedule.save();
        return { id, success: true, schedule };
      })
    );
    ...
    if (failedUpdates.length > 0) {
      return res.status(207).json({ // 207 Multi-Status
        message: 'Algunos horarios no se actualizaron',
        results,
        failedUpdates
      });
    }
    ...
    res.json({
      message: 'Todos los horarios se actualizaron correctamente',
      results
    });
